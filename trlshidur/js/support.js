function supportHandle() {
	janus.attach({
		...
		success: function(pluginHandle) {
			...
			myusername = "bb_shidur";
			var register = { "request": "join", "room": 1000, "ptype": "publisher", "display": myusername};
				...
				if(event === "joined") {
					...
					if(msg["publishers"] !== undefined && msg["publishers"] !== null) {
						...
						for(var f in list) {
							...
							// We are not going to talk with myself
							if(supuserstatus !== "shidur") {	
...
function getSupListener(id, display) {
	var dparse = display.split("_");
        supusername = dparse[0];
        supuserstatus = dparse[1];
	// Chek who enter to room
	if(supuserstatus === "bb") {
		console.log("-- ::BB translator in the room");
		$('#sessions-list').append('<a id="'+id+'" class="list-group-item list-group-item-info">'+supusername+'</a>');
	} else if(supuserstatus === "shidur") {
		console.log("-- ::Shidur modrator in the room");
	} else {
		console.log("-- ::Tranlator in the room");
		$('#sessions-list').append('<a id="'+id+'" class="list-group-item list-group-item-info">'+supusername+'</a>');
		$('#supusername').removeClass('hide').html(supusername+" - need support").show();
		notifyMe(supusername, "NEED SUPPORT");
	}
}
	...
	janus.attach({
		...
		oncleanup: function() {
		    console.log(" ::: Got a cleanup notification (remote feed " + id + ") :::");
				//$.get("index.php?action=rem&id="+id);
